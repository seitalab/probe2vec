...
from gensim_any2vec import BaseWordEmbeddingsModel
...
class Word2Vec(BaseWordEmbeddingsModel):
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
        ...
        # gensim.models.Word2Vec(gene_pairs, size=dimension, window=window_size, min_count=1, workers=num_workers, iter=1, sg=sg)
        """
        ...
        sentences : iterable of iterables
            Bag-of-wordsで入るList of List.
            Gene2vecの場合は，共発現pairのList
        ...
        sg : int {1, 0}
            ...
            gene2vecではskip-gram
        size : int
            Embeddingの次元（gene2vecでは100）
        window : int
            Windowの大きさ（gene2vecでは1）.
        alpha : float
            The initial learning rate. （gene2vecではdefault）
        min_alpha : float
            Learning rate will linearly drop to `min_alpha` as training progresses.
        seed : int
            random seed値（gene2vecではdefault）
        min_count : int
        ...
        workers : int
            ...
            (gene2vecでは32, 計算機の能力次第)
        hs : int {1,0}
            If 1, hierarchical softmax will be used for model training.
            If set to 0, and `negative` is non-zero, negative sampling will be used.
            (gene2vecではdefault, negetiveもdefault (=5)なのでnegative samplingを使用している)
        negative : int
        ...
        sorted_vocab : int {1,0}
            ...
            defaultなのでsortを行っている（頻度順にidを割り振る）
        batch_words : int
            Target size (in words) for batches of examples passed to worker threads (and
            thus cython routines).(Larger batches will be passed if individual
            texts are longer than 10000 words, but the standard cython code truncates to that maximum.)
            MAX_WORDS_IN_BATCHはあらかじめ決まった値（gene2vecではvocabサイズが小さいので問題にならないはず）
        compute_loss: bool
